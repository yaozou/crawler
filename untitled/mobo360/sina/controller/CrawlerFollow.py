...
def follow_main( driver, user_list):
    # 获得用户信息
    time.sleep(10)
    for user in user_list:
        try :
            uid = user.sinaNo
            print uid
            get_follow_from_person_info(user.sinaNo, driver)
            time.sleep(2)
            analysis_follow(driver)
            page_follow(driver)
            print user.sinaNo
        except Exception:
            continue
...
# 通过个人首页跳转粉丝页面
def get_follow_from_person_info(uid,driver):
    nextHref = 'http://weibo.com/'+uid+'/follow?rightmod=1&wvr=6'
...
if __name__ == '__main__':
    ...
    time.sleep(3)
    user_list = sina_user.UserInfo.objects.filter(crTime__istartswith='2016-12-06', referFlag='100505')
    follow_main(driver, user_list)